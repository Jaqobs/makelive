...
def add_asset_id_to_quicktime_file(filepath: str | os.PathLike, asset_id: str) -> str | None:
    ...
    with objc.autorelease_pool():
        ...
        movie, open_error = AVFoundation.AVMutableMovie.movieWithURL_options_error_(url, None, None)
        if movie is None:
            return f"Could not open {filepath} as AVMutableMovie: {open_error.description() if open_error else 'unknown error'}"
